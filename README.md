...
llms.txt                summary + section links, for LLM crawlers
...
`llms.txt` is the same idea for LLM crawlers instead of search engines —
a short summary plus links to the real pages, in the informal `llms.txt`
convention some AI crawlers look for.

## SEO structured data & headings

- `index.html`'s hero never had a real `<h1>` — the big three-line
  marquee ("Authentic / Memorabilia / Priced for fans") was just styled
  `<div>`s. It's now wrapped in a `<h1 class="premarq__h1">`; the divs
  inside keep every bit of their existing visual styling, only the
  semantics changed.
- `about.html`, `faq.html`, `refunds.html` and `shop.html` each got a
  `.pagehead__cta` button (mostly "Shop now"; `shop.html`'s links to
  `#stock`, the grid on the same page) placed right after the page's
  lede, specifically so a call-to-action is visible without scrolling
  on a phone screen, not just the headline.
- Those same four pages plus the homepage carry a JSON-LD
  `<script type="application/ld+json">` in `<head>`: an `Organization`
  block (name, url, logo, description, support email, the real
  Instagram link already in the footer) on `index.html`, `about.html`,
  `refunds.html` and `shop.html`; `faq.html` gets `FAQPage` instead,
  generated by walking every real `<summary>`/`<div class="qa__body">`
  pair already on the page — nothing invented, just marked up. If a
  question is added or reworded in the FAQ's HTML, the JSON-LD block
  needs the same edit made by hand; it isn't generated at build time.
- `shop.html` picked up a genuine "How reserving works" block (how
  saving a card differs from being charged, what's folded into the
  price, the final-sale policy) below the grid — real, already-stated
  facts, written out in full rather than just linked, mainly to give
  the page enough real static text to describe itself properly since
  the product grid itself is rendered client-side by `shop.js` after a
  fetch, not present in the page's own HTML.
