  ...
  if (resendKey) {
    ...
    try {
      await resend.emails.send({
        ...
        text: `${name} <${email}> just reserved ${productName} (${priceStr}) — no payment collected yet.\n\nShip to: ${shippingAddress}\n\nWhen it's packed and shipped: create a Stripe invoice for ${priceStr} — the listed price is all-inclusive (standard shipping, tax, fees, insurance), so don't add anything on top unless this buyer separately asked you for expedited shipping, in which case add that cost — to ${email} (Stripe Dashboard → Invoices → Create invoice, or ask Claude to do it via the Stripe connector) and send it. That's the moment they're actually charged.`,
      });
    } catch (err) {
